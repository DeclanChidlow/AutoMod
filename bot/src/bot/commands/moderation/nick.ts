...
export default {
	...
	run: async (message: MessageCommandContext, args: string[]) => {
		try {
			...
			if (!message.member.hasPermission(message.member.server!, "ManageNicknames") && !(await isModerator(message))) return message.reply(NO_MANAGER_MSG);
			...
			if (!newName) {
				...
				await message.reply(`Nickname of \`@${targetUser.username}\` has been changed to ` + `"${newName.replace(/`/g, "\\`")}".`);
			}
		} catch (e) {
			console.error("" + e);
