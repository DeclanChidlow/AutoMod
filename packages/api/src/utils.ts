	...
	for (const key of Object.keys(obj)) {
		...
		switch (structure[key]) {
			...
			case "float":
				...
				if (structure[key] == "number" && `${Math.round(obj[key])}` != `${obj[key]}`) throw `Property '${key}' was expected to be of type '${structure[key]}', got 'float' instead`;
			...
			case "strarray":
				...
				for (const i in obj[key]) {
					...
					if (typeof item != "string") throw `Property '${key}' was expected to be of type 'string[]', ` + `found '${typeof item}' at index ${i}`;
				}
