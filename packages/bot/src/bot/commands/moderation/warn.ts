	...
	run: async (message, args, serverConfig) => {
		...
		for (const userStr of targetInput) {
			try {
				...
				console.error(e);
				embeds.push(embed(`Failed to warn target \`${sanitizeMessageContent(userStr).trim()}\`: ${e}`, "Failed to warn: An error has occurred", EmbedColor.Error));
			}
		}
		...
		for (const user of targetUsers) {
			...
			await Promise.all([
				logModAction("warn", message.serverContext, message.member!, user.id, reason || "No reason provided", infraction._id, `This is warn number ${userWarnCount} for this user.`),
				(async () => {
