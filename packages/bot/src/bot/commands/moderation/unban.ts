...
export default {
	...
	run: async (message: MessageCommandContext, args: string[]) => {
		...
		try {
			let [msg, bans] = await Promise.all([message.reply("Fetching bans...")!, message.serverContext.fetchBans()]);
			...
			await Promise.all([msg.edit({ content: `User found: @${ban.user?.username ?? ban.id.user}, unbanning...` }), message.serverContext.unbanUser(id), checkTempBans(id)]);
