			...
			if (await messageContentTrigger(message, rule.trigger)) {
				for (const action of rule.action) {
					...
					switch (action.action) {
						case "sendMessage":
							if (member.hasPermission(message.channel!, "SendMessage")) await custom_sendMessage(message, action);
							else console.warn(`Custom rule ${rule._id}: 'sendMessage' action lacks permission`);
							break;
						case "delete":
							if (member.hasPermission(message.channel!, "ManageMessages")) await custom_delete(message, action);
							else console.warn(`Custom rule ${rule._id}: 'delete' action lacks permission`);
							break;
						case "warn":
							if (member.hasPermission(message.channel!, "SendMessage")) await custom_warn(message, action);
							else console.warn(`Custom rule ${rule._id}: 'warn' action lacks permission`);
							break;
						default:
