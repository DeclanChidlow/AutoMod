...
async function execute(message: Message, action: CustomRuleAction) {
	let warnMsg = action.text ? `${action.text}\n(Triggered on ${message.channelId} / ${message.id})` : `Moderation rule triggered on ${message.channelId} / ${message.id}`;
