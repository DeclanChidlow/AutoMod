...
import { client } from "../../..";
import CustomRuleTrigger from "automod-lib/dist/types/antispam/CustomRuleTrigger";
import VM from "vm";
...
async function messageContentTrigger(message: Message, trigger: CustomRuleTrigger): Promise<boolean> {
	...
	if (trigger.matcher) {
		if (trigger.channelFilter) {
			if (trigger.channelFilter.mode == "WHITELIST" && (!message.channelId || !trigger.channelFilter.channels.includes(message.channelId))) return false;
			if (trigger.channelFilter.mode == "BLACKLIST" && message.channelId && trigger.channelFilter.channels.includes(message.channelId)) return false;
		}
		...
		if (trigger.matcher instanceof RegExp) {
			...
			try {
				...
				let script = new VM.Script("matchedStrings = content.match(regex);");
				// Use runInContext with timeout option
				script.runInContext(ctx, { timeout: 2 });
