...
wsEvents.on("req:getUser", async (data: { user: string }, cb: (data: WSResponse) => void) => {
	try {
		...
		if (!user) cb({ success: false, statusCode: 404, error: "User could not be found" });
		else cb({ success: true, user: { id: user.id, username: user.username, avatarURL: user.avatarURL } as APIUser });
	} catch (e) {
		console.error(e);
		cb({ success: false, error: `${e}` });
	}
