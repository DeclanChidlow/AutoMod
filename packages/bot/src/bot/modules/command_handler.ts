...
const USER_CONFIG_CACHE_TTL = 60_000;
const userConfigCache = new Map<string, { data: any; expires: number }>();
		...
		try {
			...
			let [config, userConfig] = await Promise.all([
				...
				(async () => {
					...
					const data = await dbs.USERS.findOne({ id: msg.authorId });
					if (data) userConfigCache.set(msg.authorId!, { data, expires: Date.now() + USER_CONFIG_CACHE_TTL });
					return data;
				})(),
			]);
			...
			try {
				...
				console.error(e);
				message.reply("### An error has occurred\nAn internal error occurred while processing your command. Please try again later.");
			}
		} catch (e) {
